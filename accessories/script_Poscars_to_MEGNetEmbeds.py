...
class MEGNetShortModel(nn.Module, IOMixIn):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_proj = MLP(
            ...
            dims=[2 * 2 * dim_blocks_out + dim_blocks_out, *hidden_layer_sizes_output], # VSCK: removed output dim
            activation=activation,
            activate_last=False,
        )
    ...
    def forward(
        ...
        # VSCK: No need to sigmoid, since output is second-last layer
...
if __name__ == "__main__":
    ...
    pos_names = list(data_dict.keys())
    poscars = [dataset_path+f'/{pos}' for pos in pos_names]
    # NOTE: This is the order in which L1_Xs.npy is created
    structures = [Structure.from_file(poscar) for poscar in poscars]
    matgl_megnet_embeds = []
    for astr in structures:
        out_embeds = megnet_short_model.predict_structure(astr)
        matgl_megnet_embeds.append(out_embeds.numpy())

    matgl_megnet_embeds = np.array(matgl_megnet_embeds, dtype='float32')
    np.save(save_embeds_name, matgl_megnet_embeds)
