...
"""
This module contains a MEGNetShortModel which basically outputs the 
second last layer embeddings in a MEGNet model. We load a pre-trained 
universal MEGNet model, and create a MEGNetShortModel with same weights
to get the MEGNet structure embeddings to use in our NCVAE framework.
"""
"""
...
# Set the environment variable TF_CPP_MIN_LOG_LEVEL to 3 to suppress TensorFlow warnings.
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'

# Specify that you want to use only CPUs.
tf.config.experimental.set_visible_devices([], 'CPU')

class MEGNetShortModel(GraphModel):
    """
...
def make_megnet_short_model(
    ...
    **kwargs,
    ...
    model = Model(inputs=[x1, x2, x3, x4, x5, x6, x7], outputs=final_vec)
    return model



def generate_megnet_embedding(structures):
    """
    Loads the pre-trained MEGNet model (specifically FormationEnergy model with 2019 materialsproject dataset)
    Creates a Short model and calculates the embeddings
    
    Returns a embeddings array of shape (B, 16)
    B is the no. of structures in the astrs (list length)
    """
    megnet_Ef_path = '/home/vkolluru/GenerativeModeling/megnet_graphs/mvl_models/mp_2019_models/formation_energy.hdf5'
    megnet_Ef_model = MEGNetModel.from_file(megnet_Ef_path)

    # Create MEGNet short model with same weights
    nfeat_bond = 100
    r_cutoff = 5
    gaussian_centers = np.linspace(0, r_cutoff + 1, nfeat_bond)
    gaussian_width = 0.5
    graph_converter = CrystalGraph(cutoff=r_cutoff)
    megnet_short_model = MEGNetShortModel(graph_converter=graph_converter, centers=gaussian_centers, width=gaussian_width)

    # Set weights from the loaded megnet model 
    # NOTE: The model layers should be same for both models
    short_model_inds = [i+1 for i in range(len(megnet_short_model.layers)-1)]
    ef_model_inds = [i for i in range(len(megnet_short_model.layers)-1)]
    for i, j in zip(short_model_inds, ef_model_inds):
        try:
            megnet_short_model.layers[i].set_weights(megnet_Ef_model.layers[j].get_weights())
        except:
            print (megnet_short_model.layers[i].name, megnet_Ef_model.layers[j].name)
            continue

    # Get the embedding layer output (second last) for each structure in dataset
    embedding_vectors = megnet_short_model.predict_structures(structures)
